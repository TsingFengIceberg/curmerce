...
export default function MerchantOrdersPage() {
  ...
  const { density, setDensity } = useTableDensity("merchant-orders");
  const [orderNoInput, setOrderNoInput] = useState(orderNo);
  const [selected, setSelected] = useState<Map<number, MerchantOrder>>(new Map());
  ...
  }, [router, pageNo, status, orderNo, dateFrom, dateTo, sort]);

  useEffect(() => setOrderNoInput(orderNo), [orderNo]);

  async function load() {
    setLoading(true);
    setError(null);
    try {
      const page = await adminOrderApi.pageOwn({ pageNo, pageSize: PAGE_SIZE, status, orderNo, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined });
  ...
  function search(event: FormEvent<HTMLFormElement>) {
    ...
    update({ page: 1, orderNo: orderNoInput.trim() });
  }
  ...
  async function ship() {
    ...
    try {
      ...
      if (failures.length) {
        ...
        notifyFeedback({ tone: "error", title: "批量发货部分失败", description: `${failures.length} 笔订单仍保留在选择中`, duration: 0 });
      } else {
        setMessage(shippingRows.length === 1 ? "订单已发货" : `${shippingRows.length} 笔订单已批量发货`);
        notifyFeedback({ tone: "success", title: shippingRows.length === 1 ? "订单已发货" : `${shippingRows.length} 笔订单已批量发货` });
      }
      setShippingRows([]);
      setSelected(new Map(failures.map((failure) => [failure.row.order.id, failure.row.order])));
  ...
  const selectedOrders = Array.from(selected.values()).filter((order) => order.status === 20);
  const pagePendingOrders = orders.filter((order) => order.status === 20);
  const allPendingSelected = pagePendingOrders.length > 0 && pagePendingOrders.every((order) => selected.has(order.id));
  ...
  return (
    <section className="content-section merchant-orders-page">
      ...
      <div className="workspace-section merchant-order-table-panel">
        <div className="order-admin-toolbar"><select aria-label="订单状态" value={status ?? ""} onChange={(event) => update({ status: event.target.value, page: 1 })}><option value="">全部状态</option><option value="10">待支付</option><option value="20">待发货</option><option value="30">已发货</option><option value="40">已完成</option><option value="50">已取消</option></select><form className="order-search" onSubmit={search}><Search aria-hidden="true" size={16} /><input aria-label="订单号" placeholder="搜索订单号" value={orderNoInput} onChange={(event) => setOrderNoInput(event.target.value)} /><button type="submit">查询</button></form><select aria-label="当前页排序" value={sort} onChange={(event) => update({ sort: event.target.value })}><option value="created-desc">最新下单</option><option value="created-asc">最早下单</option><option value="amount-desc">金额从高到低</option></select><TableDensityControl density={density} onChange={setDensity} /><span>共 {total} 笔</span></div>
        <div className="date-range-toolbar"><label><span>开始日期</span><input type="date" value={dateFrom} onChange={(event) => update({ dateFrom: event.target.value, page: 1 })} /></label><label><span>结束日期</span><input min={dateFrom} type="date" value={dateTo} onChange={(event) => update({ dateTo: event.target.value, page: 1 })} /></label>{dateFrom || dateTo ? <button className="text-button" type="button" onClick={() => update({ dateFrom: null, dateTo: null, page: 1 })}>清除日期</button> : null}{selectedOrders.length ? <span>已跨页选择 {selectedOrders.length} 笔待发货订单</span> : null}</div>
