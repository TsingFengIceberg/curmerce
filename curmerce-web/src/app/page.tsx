...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    ]).then(([productPage, personalPage, postPage, releasePage, auctionPage]) => {
      setProducts(productPage?.list ?? []);
      setPersonalProducts(personalPage?.list ?? []);
      setPosts(postPage?.list ?? []);
      setReleases(releasePage?.list ?? []);
      setAuctions(auctionPage?.list ?? []);
  ...
  return (
    <div className="discovery-home">
      ...
      <section className="discovery-section">
        ...
        {loading ? <div className="content-skeleton-grid">{Array.from({ length: 4 }, (_, index) => <span key={index} />)}</div> : posts.length === 0 ? <EmptyState title="还没有公开分享" action={{ href: "/community/create", label: "发布第一篇帖子" }} /> : <div className="home-story-grid">{posts.map((post) => <Link className="home-story" href={`/community/${post.id}`} key={post.id}>{post.mediaUrls?.[0] ? <img src={assetUrl(post.mediaUrls[0]) ?? ""} alt="" /> : <div className="home-story__placeholder"><Sparkles aria-hidden="true" /></div>}<div><span>{post.authorNickname || "Curmerce 用户"} · {formatDateTime(post.createTime)}</span><h3>{post.title}</h3><p>{post.content}</p><small>{post.likeCount} 赞 · {post.commentCount} 评论</small></div></Link>)}</div>}
      </section>
      ...
      <section className="discovery-section">
        ...
        {loading ? <div className="content-skeleton-grid">{Array.from({ length: 4 }, (_, index) => <span key={index} />)}</div> : products.length === 0 ? <EmptyState title="暂时没有在售商品" action={{ href: "/community", label: "先逛社区" }} /> : <div className="product-grid home-product-grid">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div>}
      </section>
      ...
      <section className="discovery-section">
        ...
        {loading ? <div className="content-skeleton-grid">{Array.from({ length: 4 }, (_, index) => <span key={index} />)}</div> : personalProducts.length === 0 ? <EmptyState title="暂时没有新的个人闲置" description="个人卖家发布并通过审核后，会在这里展示。" action={{ href: "/personal/listings/new", label: "发布一件闲置" }} /> : <div className="product-grid home-product-grid">{personalProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div>}
      </section>

      <section className="discovery-section">
        <div className="discovery-section__heading"><div><p className="eyebrow">SPECIAL COMMERCE</p><h2>限时与稀缺</h2></div></div>
        <div className="home-event-grid">
          {releases.length ? releases.map((campaign) => <Link className="home-event home-event--release" href="/releases" key={campaign.id}><span><Timer aria-hidden="true" size={18} />限时发售</span><h3>{campaign.name}</h3><p>{formatDateTime(campaign.startTime)} 开始 · 每人限购 {campaign.perUserLimit} 件</p><strong>{campaign.items[0] ? formatMoney(campaign.items[0].campaignPrice) : "查看活动"}</strong></Link>) : <Link className="home-event home-event--release" href="/releases"><span><Timer aria-hidden="true" size={18} />限时发售</span><h3>等待下一场限时活动</h3><p>活动开放后，可选择具体 SKU 和收货地址参与购买。</p><strong>查看活动日历</strong></Link>}
          {auctions.length ? auctions.map((auction) => <Link className="home-event home-event--auction" href="/auctions" key={auction.id}><span><Gavel aria-hidden="true" size={18} />公开拍卖</span><h3>{auction.name}</h3><p>{formatDateTime(auction.endTime)} 结束 · 最低加价 {formatMoney(auction.minIncrement)}</p><strong>{auction.currentAmount == null ? `起拍 ${formatMoney(auction.startingPrice)}` : `当前 ${formatMoney(auction.currentAmount)}`}</strong></Link>) : <Link className="home-event home-event--auction" href="/auctions"><span><Gavel aria-hidden="true" size={18} />公开拍卖</span><h3>等待下一件稀缺商品</h3><p>拍卖发布后，可查看商品详情、竞价记录和当前领先状态。</p><strong>进入拍卖场</strong></Link>}
        </div>
      </section>
    </div>
  );
