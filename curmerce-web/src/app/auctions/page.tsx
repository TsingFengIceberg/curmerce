    ...
    <section className="content-section commerce-event-page commerce-event-page--productized">
      ...
      {!loading && sessions.length ? <div className="auction-list">{sessions.map((session) => {
        ...
        return <article className="auction-card" key={session.id}>
          <div className="auction-card__visual">{session.productImageUrl ? <img alt={session.productName ?? session.name} src={assetUrl(session.productImageUrl) ?? ""} /> : <span><PackageSearch aria-hidden="true" size={32} /></span>}<span className="tag auction-card__status">{labels[session.status] ?? `状态 ${session.status}`}</span></div>
