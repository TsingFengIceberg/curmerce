...
export default function CommunityPage() {
  ...
  async function load() {
    ...
    try { setPosts((await communityApi.page({ pageNo: 1, pageSize: 20, keyword })).list ?? []); }
  ...
  return <section className="content-section community-page">
    <div className="section-heading"><div><p className="eyebrow">COMMUNITY · DISCOVER</p><h1>从真实分享，找到下一件喜欢的东西。</h1><p>浏览兴趣内容，查看关联商品，并在同一条发现路径里完成交易。</p></div><Link className="button button--primary" href="/community/create">发布帖子</Link></div>
    <form className="community-search" onSubmit={(event) => { event.preventDefault(); void load(); }}><input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索帖子标题或内容" /><button className="button button--secondary" type="submit">搜索</button></form>
