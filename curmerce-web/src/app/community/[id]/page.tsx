  ...
  return (
    <section className="content-section community-detail community-detail--productized">
      ...
      <article className="community-detail__article community-article">
        ...
        {post.mediaUrls?.length ? <div className={`community-media-grid community-media-grid--${Math.min(post.mediaUrls.length, 4)}`}>{post.mediaUrls.map((url, index) => <button aria-label={`查看第 ${index + 1} 张配图`} key={url} type="button" onClick={() => openPreview(url)}><MediaImage src={assetUrl(url)} alt={`帖子配图 ${index + 1}`} /><span><Eye aria-hidden="true" size={18} /></span></button>)}</div> : null}
        ...
        {post.products?.length ? <div className="community-products community-products--compact"><div className="panel-heading"><h2>帖子中提到的商品</h2><span>{post.products.length} 件</span></div><div className="community-product-strip">{post.products.map((product) => <Link href={`/products/${product.id}`} key={product.id}><MediaImage alt={product.name} fallback={<span className="listing-table__placeholder">C</span>} src={assetUrl(product.mainImageUrl)} /><div><strong>{product.name}</strong><small>{product.storeName || "个人卖家"}</small><b>{formatMoney(product.minPrice)}</b></div></Link>)}</div></div> : null}
      </article>
