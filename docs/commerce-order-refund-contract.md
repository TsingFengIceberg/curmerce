...
仍未完成的生产化缺口：支付与退款回调仍为本地模拟接口，没有真实支付渠道签名验签和渠道凭证校验；死信目前是库内 DEAD 标记而非外部 DLQ；对账发现的问题需要人工介入，还没有自动补偿与修复流程。服务拆分后必须补齐跨服务事件、幂等消费、超时、重试、外部死信和自动修复，并保证 Outbox 发布器与消费者在独立进程下不丢事件、不重复处理。
