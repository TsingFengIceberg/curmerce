...
public class AgentRetrievalService {
    ...
    public AgentAssistRespDTO assist(String query) {
        ...
        JsonNode products = fetch(coreClient, "/app-api/commerce/catalog/product-page", query, "core", degraded);
        JsonNode posts = fetch(communityClient, "/app-api/community/post/page", query, "community", degraded);
    ...
    private JsonNode fetch(RestClient client, String path, String query, String source, List<String> degraded) {
        try {
            CommonResult<JsonNode> response = client.get().uri(uri -> uri.path(path)
            ...
            degraded.add(source);
            log.warn("agent retrieval source unavailable: source={}, reason={}", source, ex.getMessage());
            return JsonNodeFactory.instance.objectNode();
        }
    }
